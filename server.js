...
function buildDeck() {
  ...
  COLORS.forEach(color => {
    VALUES.forEach(value => {
      // 0 appears once, others twice
      const count = value === '0' ? 1 : 2;
      for (let i = 0; i < count; i++) {
        deck.push({ id: id++, color, value, type: 'normal' });
      }
    });
    // draw4 and wild x2 each per color set
  });
  // Wild cards (4 each)
...
function createRoom(roomId) {
  return {
    id: roomId,
    players: {},        // pid -> { name, color, hand, ready }
    ...
    discard: [],
    topCard: null,
    activeColor: null,  // for wild
    started: false,
    winner: null,
    scores: {},
    round: 1,
    drawStack: 0,       // accumulated draw2/draw4
    unoCallers: new Set(),
  };
}
...
function drawCards(room, pid) {
  ...
  // Check if drawn card is playable (only if drawStack was 0)
...
server.listen(PORT, () => console.log(`🃏 UNO server: http://localhost:${PORT}`));
